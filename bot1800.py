...
c = 1
allTables = ["30sec","1min","2min","3min","5min","10min","15min","30min"]

def myround(x, base=1800): 
...
while True:
	try:
		...
		if x == counter:
			...
			cursor = db.cursor()
			cursor.execute("INSERT INTO 30min (num1,coin1,num2,coin2,num3,coin3,num4,coin4,num5,coin5,num6,coin6,num7,coin7,num8,coin8,num9,coin9,num10,coin10, date) VALUES (" 
			...
			for l in allTables:
				delCursor = db.cursor()
				delCursor.execute("DELETE FROM " + l + " WHERE DATE < DATE_SUB(NOW(), INTERVAL 1 DAY)")
				db.commit()
